...
RSpec.describe "imap-backup with ERB configuration", :container, type: :aruba do
  ...
  after do
    ...
    test_server.disconnect
    FileUtils.rm_f(erb_config_path)
  ...
  context "when both --config and --erb-configuration are supplied" do
    ...
    let(:command) do
      "imap-backup stats #{account[:username]} " \
        "--config #{config_path} " \
        "--erb-configuration #{erb_config_path}"
    end
    ...
    it "fails with an error" do
      ...
      expect(last_command_started).
        to have_output(/Cannot specify both --config and --erb-configuration/)
    end
  end
  ...
  context "when ERB template file does not exist" do
    ...
    let(:command) do
      "imap-backup stats #{account[:username]} --erb-configuration #{missing_erb_path}"
    end
  ...
  context "when setup command is used with --erb-configuration" do
    ...
    it "does not recognize the option" do
      ...
      expect(last_command_started).
        to have_output(
          /ERROR: "imap-backup setup" was called with arguments \["--erb-configuration"/i
        )
    end
  end
end
